...
case class Assembly1(name: String) extends Assembly
  ...
  def startHttpServer(): Unit = {
    ...
    val settings = Assembly1Settings(context.system)
    context.actorOf(CommandServiceHttpServer.props(context.parent,
      settings.interface, settings.port, settings.timeout), "commandService")
  }
